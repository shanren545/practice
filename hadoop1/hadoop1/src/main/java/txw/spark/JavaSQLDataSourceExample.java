...
public class JavaSQLDataSourceExample {
    ...
    private static void runBasicDataSourceExample(SparkSession spark) {
        ...
        Dataset<Row> partitionedUsersDF =
                spark.read().format("orc").option("pathGlobFilter", "*.orc").load("examples/src/main/resources/partitioned_users.orc");
    ...
    private static void runBasicParquetExample(SparkSession spark) {
        ...
        Dataset<Row> peopleDF = spark.read().json("src/main/resources/people.json").cache();
        ...
        Dataset<Row> namesDF = spark.sql("SELECT name FROM parquetFile WHERE age BETWEEN 1 AND 19");
        Dataset<String> namesDS = namesDF.map((MapFunction<Row, String>) row -> "Name2: " + row.getString(0), Encoders.STRING());
        namesDS.show();
        // +------------+
        // | value|
        // +------------+
    ...
    private static void runParquetSchemaMergingExample(SparkSession spark) {
        ...
        long count = mergedDF.count();
        mergedDF.show();
        mergedDF.write().json("mergedDF.json");
