 ...
 * 背包问题
 * 
 * https://blog.csdn.net/Jinlong_Xu/article/details/96840286
 *
 * @author tanxianwen 2019年12月6日
 */
public class Backpack {
    ...
    /**
     * 0-1背包的优化解法 思路： 只用一个一维数组记录状态，dp[i]表示容量为i的背包所能装入物品的最大价值 用逆序来实现
     */
    ...
    /**
     * 第二类背包：完全背包 思路分析： 01背包问题是在前一个子问题（i-1种物品）的基础上来解决当前问题（i种物品），
     * 向i-1种物品时的背包添加第i种物品；而完全背包问题是在解决当前问题（i种物品） 向i种物品时的背包添加第i种物品。 推公式计算时，f[i][y] = max{f[i-1][y],
     * (f[i][y-weight[i]]+value[i])}， 注意这里当考虑放入一个物品 i 时应当考虑还可能继续放入 i，
    ...
    /**
     * 完全背包的第二种解法 思路： 只用一个一维数组记录状态，dp[i]表示容量为i的背包所能装入物品的最大价值 用顺序来实现
     */
